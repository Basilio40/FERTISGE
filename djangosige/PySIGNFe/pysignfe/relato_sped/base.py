...
from djangosige.geraldo.geraldo import Report, ReportBand, BAND_WIDTH
from djangosige.geraldo.geraldo import ObjectValue, SystemField, Label, Line
import os
...
FONTES_ADICIONAIS = {
    u'Gentium Book Basic': (
        ...
        (u'Courier New'                   , DIRNAME + u'/fonts/cour.ttf', False , False),
        (u'Courier New Italic'            , DIRNAME + u'/fonts/couri.ttf', False , True),
        (u'Courier New Bold'              , DIRNAME + u'/fonts/courbd.ttf', True , False),
        (u'Courier New Bold Italic'       , DIRNAME + u'/fonts/courbi.ttf', True , True),
    )
}
...
FONTE_NORMAL = 'Gentium Book Basic' if fonte else 'Courier New'
...
DADO_CHAVE = ({'fontName': FONTE_NEGRITO, 'fontSize': FONTE_TAMANHO_85, 'alignment': TA_CENTER} if fonte
              else {'fontName': FONTE_NEGRITO, 'fontSize': FONTE_TAMANHO_6, 'alignment': TA_CENTER})
DADO_VARIAVEL = {'fontName': FONTE_NORMAL, 'fontSize': FONTE_TAMANHO_9, 'alignment': TA_CENTER, 'leading': FONTE_TAMANHO_11}

DADO_CAMPO = ({'fontName': FONTE_NORMAL, 'fontSize': FONTE_TAMANHO_10, 'leading': FONTE_TAMANHO_12} if fonte
              else {'fontName': FONTE_NORMAL, 'fontSize': FONTE_TAMANHO_8, 'leading': FONTE_TAMANHO_10})
DADO_CAMPO_NEGRITO = ({'fontName': FONTE_NEGRITO, 'fontSize': FONTE_TAMANHO_10, 'leading': FONTE_TAMANHO_12} if fonte
                      else {'fontName': FONTE_NEGRITO, 'fontSize': FONTE_TAMANHO_8, 'leading': FONTE_TAMANHO_10})
DADO_CAMPO_NUMERICO = ({'fontName': FONTE_NORMAL, 'fontSize': FONTE_TAMANHO_10, 'alignment': TA_RIGHT, 'leading': FONTE_TAMANHO_12} if fonte
                       else {'fontName': FONTE_NORMAL, 'fontSize': FONTE_TAMANHO_8, 'alignment': TA_RIGHT, 'leading': FONTE_TAMANHO_10})
DADO_CAMPO_NUMERICO_NEGRITO = ({'fontName': FONTE_NEGRITO, 'fontSize': FONTE_TAMANHO_10, 'alignment': TA_RIGHT, 'leading': FONTE_TAMANHO_12} if fonte
                               else {'fontName': FONTE_NEGRITO, 'fontSize': FONTE_TAMANHO_8, 'alignment': TA_RIGHT, 'leading': FONTE_TAMANHO_10})
...
class BandaDANFE(ReportBand):
    ...
    def inclui_campo_produto(self, nome, conteudo, top, left, width, height=None, margem_direita=False, centralizar=False):
        fld = self._inclui_campo(nome, conteudo, top, left, width, height)
        ...
        if(centralizar):
            fld.style.update({'alignment': TA_CENTER})
        else:
            fld.style.update({'alignment': TA_LEFT})
