...
from djangosige.geraldo.geraldo import Report, ReportBand, SubReport
from djangosige.geraldo.geraldo import ObjectValue, SystemField, Label, Line, Rect, Image
from djangosige.geraldo.geraldo.barcodes import BarCode
from djangosige.geraldo.geraldo.generators import PDFGenerator

from djangosige.PySIGNFe.pysignfe.relato_sped import *
from djangosige.PySIGNFe.pysignfe.nfe.manual_401 import Vol_200
...
class RemetenteRetrato(BandaDANFE):
    ...
    def dados_emitente_logo_vertical(self, arquivo_imagem):
        ...
        a_ratio = img._get_width() / img._get_height()
        img.height = 116
        w =  116 * a_ratio
        img.width = w if (w <= 191) else 191
        
        img.left = 191 - w / 2 * 0.1*cm if w < 191 else 0.1*cm
        img.top = 0.1*cm
...
class CabProdutoRetrato(BandaDANFE):
    def __init__(self):
        ...
        lbl = self.inclui_descritivo_produto(nome='', titulo='DESCRIÇÃO DO PRODUTO/SERVIÇO', top=0.42*cm, left=2*cm, width=5.31*cm) #left=2*cm, width=4.31*cm
        lbl.padding_top = 0.15*cm
        #lbl = self.inclui_descritivo_produto(nome='', titulo='CÓDIGO DE BARRAS', top=0.42*cm, left=6.3*cm, width=1.57*cm)
        #lbl.padding_top = 0.15*cm
        lbl = self.inclui_descritivo_produto(nome='', titulo='NCM/SH', top=0.42*cm, left=7.3*cm, width=1.2*cm) #left=7.86*cm, width=1*cm
        lbl.padding_top = 0.15*cm
        lbl = self.inclui_descritivo_produto(nome='', titulo='CST', top=0.42*cm, left=8.49*cm, width=0.79*cm) #left=8.86*cm, width=0.44*cm
        lbl.padding_top = 0.15*cm
        lbl = self.inclui_descritivo_produto(nome='', titulo='CFOP', top=0.42*cm, left=9.27*cm, width=0.68*cm) #left=9.3*cm, width=0.54*cm
        lbl.padding_top = 0.15*cm
        lbl = self.inclui_descritivo_produto(nome='', titulo='UNIDADE', top=0.42*cm, left=9.94*cm, width=1.0*cm) #left=9.84*cm, width=1.1*cm
        lbl.padding_top = 0.15*cm
...
class DetProdutoRetrato(BandaDANFE):
    def __init__(self):
        ...
        txt = self.inclui_campo_produto(nome=u'prod_codigo', conteudo=u'prod.cProd.valor', top=0*cm, left=0*cm, width=2*cm, centralizar=True)
        txt = self.inclui_campo_produto(nome=u'prod_descricaco', conteudo=u'descricao_produto_formatada', top=0*cm, left=2*cm, width=5.31*cm) #left=2*cm, width=4.31*cm)
        ...
        txt = self.inclui_campo_centralizado_produto(nome=u'prod_ncm', conteudo=u'prod.NCM.valor', top=0*cm, left=7.3*cm, width=1.2*cm) #left=7.86*cm, width=1*cm)
        txt = self.inclui_campo_centralizado_produto(nome='prod_cst', conteudo='cst_formatado', top=0*cm, left=8.49*cm, width=0.79*cm) #left=8.86*cm, width=0.44*cm)
        txt = self.inclui_campo_centralizado_produto(nome=u'prod_cfop', conteudo=u'prod.CFOP.valor', top=0*cm, left=9.27*cm, width=0.68*cm) #left=9.3*cm, width=0.54*cm)
        txt = self.inclui_campo_centralizado_produto(nome=u'prod_unidade', conteudo=u'prod.uCom.valor', top=0*cm, left=9.94*cm, width=1.0*cm) #left=9.84*cm, width=1.1*cm)
