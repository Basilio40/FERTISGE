...
class NotaFiscalForm(forms.ModelForm):
    ...
    class Meta:
        ...
        error_messages = {
            'n_nf': {
                'unique': ("Nota fiscal com este número já existe"),
            },
        }
...
class NotaFiscalSaidaForm(NotaFiscalForm):
    ...
    class Meta(NotaFiscalForm.Meta):
        ...
        labels['n_nf_saida'] = ('Número')
        labels['venda'] = ('Venda')
        labels['emit_saida'] = ('Emitente (Empresa)')
        labels['dest_saida'] = ('Destinatário (Cliente)')
        labels['n_fat'] = ('Número da fatura')
        labels['tpnf'] = ('Tipo de Operação')
        labels['v_orig'] = ('Valor original da fatura')
        labels['v_desc'] = ('Valor do desconto')
        labels['v_liq'] = ('Valor líquido da fatura')
        labels['grupo_cobr'] = (
            'Inserir dados de cobrança (Fatura/Duplicatas) na NF-e?')
        labels['arquivo_proc'] = ('Arquivo de processamento (*_procNFe.xml)')
...
class NotaFiscalEntradaForm(NotaFiscalForm):
    ...
    class Meta(NotaFiscalForm.Meta):
        ...
        labels = NotaFiscalForm.Meta.labels
        labels['n_nf_entrada'] = ('Número')
        labels['compra'] = ('Compra')
        labels['emit_entrada'] = ('Emitente (Fornecedor)')
        labels['dest_entrada'] = ('Destinatário (Empresa)')


class EmissaoNotaFiscalForm(forms.ModelForm):

    def __init(self, *args, **kwargs):
        super(EmissaoNotaFiscalForm, self).__init(*args, **kwargs)
        self.fields['dhemi'].input_formats = ('%d/%m/%Y %H:%M',)
...
class CancelamentoNotaFiscalForm(forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {
            'just_canc': ('Justificativa do cancelamento'),
            'chave': ('Chave'),
...
class AutXMLForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('cpf_cnpj',)
        labels = {
            'cpf_cnpj': ('CPF/CNPJ (Apenas digitos)'),
        }
        widgets = {
...
class ConfiguracaoNotaFiscalForm(forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {
            'arquivo_certificado_a1': ('Certificado A1'),
            'serie_atual': ('Série atual'),
            'ambiente': ('Ambiente'),
            'imp_danfe': ('Tipo de impressão DANFE'),
            'senha_certificado': ('Senha do certificado'),
            'inserir_logo_danfe': ('Inserir logo da empresa no DANFE?'),
            'orientacao_logo_danfe': ('Orientação do logo'),
            'csc': ('Código de Segurança do Contribuinte'),
            'cidtoken': ('Identificador do CSC'),
        }
        widgets = {
